...
/write|编译write.ejs模板|get|id|返回write页面|登陆后访问，有id是编辑页，无id是新增页
/detail|编译detail.ejs模板|get|id|返回detail页面|无
/user/regist|注册业务|post|username,password,password2|重定向|注册成功重定向到/login，失败重定向到/regist
/user/login|登陆业务|post|username，pqassword|重定向|登陆成功重定向到/，失败重定向到/login
/user/logout|退出登陆业务|get|无|重定向|退出登陆后重定向到/login
/articles/write|文章修改和新增业务|post|title,content,username,id|重定向|有id是修改业务，没有id是新增业务，成功重定向到/,失败重定向到/write
...
// favicon
var facicon = require('sever-favicon')
